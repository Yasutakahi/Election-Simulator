...
def clean_data(data: List[List[str]]) -> None:
    ...
    for i in data:
        ...
        for s in range(4):
            if w[s] == 'CPC':
                inner0.append('CPC')
            if w[s] == 'Green' or w[s] == 'GREEN':
                inner0.append('GREEN')
            if w[s] == 'Liberal' or w[s] == 'LIBERAL':
                inner0.append('LIBERAL')
            if w[s] == 'NDP':
                inner0.append('NDP')
        i.append(inner0)
        m = i[3].split(';')
...
def get_lowest_not_removed(votes: List[int], removed_parties: List[str]) -> str:
    ...
    'GREEN'
    """
    s = ['CPC', 'GREEN', 'LIBERAL', 'NDP']
    for i in removed_parties:
        votes.remove(votes[PARTY_ORDER.index(i)])
    for n in removed_parties:
        s.remove(n)
    return s[votes.index(min(votes))]
...
def voting_irv(rank_ballots: List[List[str]]) -> str:
    ...
    """
    removed = []
    while max(voting_borda(rank_ballots)) <= 1/2 * \
          sum(voting_borda(rank_ballots)):
        remove_party(rank_ballots, get_lowest_not_removed\
                     (voting_borda(rank_ballots), removed))
        removed.append(get_lowest_not_removed\
                       (voting_borda(rank_ballots), removed))
    return PARTY_ORDER[voting_borda(rank_ballots).index\
                       (max(voting_borda(rank_ballots)))] 
...
def points_to_winner(points_per_party: List[int]) -> str:
    ...
    'GREEN'
    """
    for i in range(len(points_per_party)):
        if points_per_party[i] == max(points_per_party):
